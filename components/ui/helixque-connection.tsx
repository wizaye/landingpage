  ...
  const UserCard = ({ user }: { user: User }) => (
    ...
    >
      ...
      <div className="px-1 sm:px-2 py-1 sm:py-1.5 text-center bg-gray-100 dark:bg-neutral-800 flex-1 flex flex-col justify-center">
        <div className="text-xs sm:text-xs text-gray-700 dark:text-neutral-300 font-medium truncate">{user.avatar}</div>
      </div>
      ...
      <div className="p-1 sm:p-2 flex justify-center gap-1 bg-gray-100 dark:bg-neutral-800 border-t border-gray-300 dark:border-neutral-700">
        <button
          className={`p-1 rounded text-xs ${user.isVideoOn ? 'bg-blue-600' : 'bg-gray-400 dark:bg-neutral-600'} transition-colors touch-manipulation`}
        ...
        <button
          className={`p-1 rounded text-xs ${user.isAudioOn ? 'bg-blue-600' : 'bg-gray-400 dark:bg-neutral-600'} transition-colors touch-manipulation`}
  ...
  const CentralSystem = ({ isActive }: { isActive: boolean }) => (
    <div className="relative mx-3 sm:mx-6">
      <motion.div
        ...
        animate={{
          borderColor: isActive ? '#3b82f6' : (theme === 'dark' ? '#525252' : '#9ca3af'),
          backgroundColor: isActive 
            ? 'rgba(59, 130, 246, 0.1)' 
            : (theme === 'dark' ? '#262626' : '#e5e7eb')
        }}
      ...
      >
        ...
        <motion.div
          ...
          animate={{
            ...
            backgroundColor: isActive ? '#2563eb' : (theme === 'dark' ? '#525252' : '#6b7280')
          }}
          transition={{ 
      ...
      <div className="absolute top-1/2 -left-3 sm:-left-6 w-3 sm:w-6 h-px">
        <motion.div
          className={`w-full h-px ${isActive ? 'bg-blue-500' : 'bg-gray-400 dark:bg-neutral-600'}`}
          initial={{ scaleX: 0 }}
