...
const HelixqueConnection = () => {
  ...
  const [connectionStatus, setConnectionStatus] = useState<'user_connecting' | 'system_activated' | 'system_searching' | 'match_found' | 'connected' | 'skipped'>('user_connecting');
  ...
  useEffect(() => {
    const cycle = async () => {
      ...
      setUser2(prev => ({ ...prev, status: 'disconnected', avatar: 'Stranger' }));
      await new Promise(resolve => setTimeout(resolve, 1500));
  ...
  const UserCard = ({ user }: { user: User }) => (
    <div
      className={`relative w-20 sm:w-28 h-28 sm:h-36 bg-neutral-800 rounded-lg border border-neutral-700 overflow-hidden flex flex-col ${
        user.status === 'disconnected' ? 'opacity-40' : 'opacity-100'
      } transition-opacity duration-500`}
    >
      {/* Video feed simulation */}
      <div className="h-14 sm:h-20 bg-gradient-to-br from-neutral-700 to-neutral-800 flex items-center justify-center relative">
        {user.isVideoOn ? (
          <div className="w-8 sm:w-10 h-8 sm:h-10 bg-neutral-600 rounded-full flex items-center justify-center">
            <User className="w-4 sm:w-5 h-4 sm:h-5 text-neutral-300" />
          </div>
        ) : (
          <VideoOff className="w-4 sm:w-5 h-4 sm:h-5 text-neutral-500" />
        )}
      ...
      {/* User info */}
      <div className="px-1 sm:px-2 py-1 sm:py-1.5 text-center bg-neutral-800 flex-1 flex flex-col justify-center">
        <div className="text-xs sm:text-xs text-neutral-300 font-medium truncate">{user.avatar}</div>
      </div>

      {/* Controls */}
      <div className="p-1 sm:p-2 flex justify-center gap-1 bg-neutral-800 border-t border-neutral-700">
        <button
          className={`p-1 rounded text-xs ${user.isVideoOn ? 'bg-blue-600' : 'bg-neutral-600'} transition-colors touch-manipulation`}
        ...
        <button
          className={`p-1 rounded text-xs ${user.isAudioOn ? 'bg-blue-600' : 'bg-neutral-600'} transition-colors touch-manipulation`}
    ...
    <div className="relative mx-3 sm:mx-6">
      <motion.div
        className={`w-16 sm:w-24 h-16 sm:h-24 rounded-xl border-2 flex items-center justify-center relative ${
          isActive ? 'border-blue-500 bg-blue-900/20' : 'border-neutral-600 bg-neutral-800'
        }`}
        animate={{
          borderColor: isActive ? '#3b82f6' : '#525252',
          backgroundColor: isActive ? 'rgba(59, 130, 246, 0.1)' : '#262626'
        }}
      ...
      >
        ...
        <motion.div
          ...
          animate={{
            ...
            backgroundColor: isActive ? '#2563eb' : '#525252'
          }}
          transition={{ 
        ...
        >
          <div className="text-white text-xs sm:text-sm font-bold">H</div>
        </motion.div>
      ...
      <div className="absolute top-1/2 -left-3 sm:-left-6 w-3 sm:w-6 h-px">
        <motion.div
          className={`w-full h-px ${isActive ? 'bg-blue-500' : 'bg-neutral-600'}`}
          initial={{ scaleX: 0 }}
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center p-2 sm:p-4 bg-black rounded-lg">
      {/* Header with status */}
      <div className="flex items-center gap-2 mb-4 sm:mb-6">
        <div className="flex items-center gap-2 text-white">
          <div className={`w-2 h-2 rounded-full ${
            connectionStatus === 'connected' ? 'bg-green-500' :
            connectionStatus === 'user_connecting' ? 'bg-yellow-500' :
            connectionStatus === 'system_activated' ? 'bg-blue-400' :
            connectionStatus === 'system_searching' ? 'bg-blue-500' :
            connectionStatus === 'match_found' ? 'bg-orange-500' : 'bg-neutral-500'
          }`} />
          <span className="text-xs sm:text-sm font-medium text-neutral-300">
            {connectionStatus === 'user_connecting' ? 'Connecting...' :
             connectionStatus === 'system_activated' ? 'Connected...' :
             connectionStatus === 'system_searching' ? 'Searching...' :
             connectionStatus === 'match_found' ? 'Connecting...' :
  ...
  <UserCard user={user1} />
        ...
        <CentralSystem isActive={connectionStatus === 'system_activated' || connectionStatus === 'system_searching' || connectionStatus === 'match_found' || connectionStatus === 'connected'} />
